...
"""
    ...
    def test_uncertain_jobs_have_plain_label_and_specific_count(self):
        ...
        self.assertEqual(badge["word"], "Needs review:")
        self.assertEqual(badge["count"], "5")
        self.assertFalse(badge["hidden"])
        self.assertIn("5 jobs need review", badge["title"])
        self.assertIn("is-uncertain", badge["classes"])

    def test_active_and_queued_count_is_named(self):
        badge = self.render_badge(
            {
                "ok": True,
                "worker": {"capabilities": ["engine-execution-v1"]},
        ...
        self.assertEqual(badge["word"], "Worker jobs:")
        self.assertEqual(badge["count"], "5")
        self.assertFalse(badge["hidden"])
