...
def test_narrow_conversation_rows_shed_secondary_chips_before_title():
    ...
    agents = source.index(".conv-item .conv-signal.subagents,\n    .conv-item .conv-signal.workflows")
    ...
    assert ".conv-subagent-cluster-toggle" not in source[source.index("@container sidebar (max-width: 740px)"):running]
