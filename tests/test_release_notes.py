...
def test_parse_git_log_output_splits_records_and_files():
    ...
    assert [c.sha for c in commits] == [
        "aaa1111", "bbb2222", "ccc3333", "ddd4444", "eee5555", "fff6666", "777aaaa",
    ]
    ...
    assert commits[5].files == ["server.py", "archive.py"]
    assert commits[6].files == ["README.md"]
...
def test_group_commits_buckets_and_groups_by_scope():
    ...
    # freeform commit all land in "under the hood".
    under_the_hood_shas = {c.sha for c in under_the_hood}
    assert under_the_hood_shas == {"ccc3333", "ddd4444", "eee5555", "777aaaa"}
...
def test_capability_renders_what_why_how():
    ...
    [(what, why)] = cap.items
    assert "no longer no-ops" in what
    assert why  # some non-empty templated sentence
    assert cap.how_files == ["ccc_worker.py"]
...
def test_render_markdown_has_value_first_headings_and_collapsed_under_the_hood():
    ...
    assert "Under the hood (4 commits)" in md
...
def test_cli_end_to_end_against_a_throwaway_git_repo(tmp_path):
    ...
    run("git", "add", "a.txt")
    run("git", "commit", "-q", "-m", "feat(widgets): add a shiny widget")
