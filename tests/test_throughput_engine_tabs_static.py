...
def test_throughput_page_exposes_claude_codex_tabs_and_passes_engine_param():
    ...
    assert "activeThroughputEngine = initialThroughputEngine()" in throughput_html
...
def test_engine_selection_persists_across_reloads():
    throughput_html = pathlib.Path(PROJECT_ROOT, "static", "throughput.html").read_text(encoding="utf-8")

    assert "const THROUGHPUT_ENGINE_STORE_KEY = 'ccc-throughput-engine'" in throughput_html
    assert "function initialThroughputEngine()" in throughput_html
    assert "localStorage.setItem(THROUGHPUT_ENGINE_STORE_KEY, next)" in throughput_html
