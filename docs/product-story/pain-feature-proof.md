...
| Per-queue learnings file | Watchtower `workers.py` ("FIRST, read the queue's learnings file...") — separate public repo, github.com/amirfish1/watchtower, installed by default via `scripts/install.sh` | Partial | Screenshot of learnings file + CLI | Medium | Public, F4 (row 22 — the family's headline row). Updated 2026-08-21 (WT-92): Watchtower is now a hard, required dependency for CCC's queue engine — there is no fallback; CCC's queue features fail loudly at startup if `watchtower.queue` isn't importable. Claim as required CCC behavior, not "ships by default with fallback." Public name: "work queue." |
| Work queue (claim/fix/verify/close) | `watchtower/queue.py` (via `server.py`'s `_q`); `/api/ux-fixes/*` | Built | Screenshot (synthetic tickets) | Low | Public, F4. Public name: "work queue." "Watchtower" is a separate product name; use sparingly. |
| Queue-health watcher + auto-nudge | `/api/ux-fixes/health`; product manifest; Watchtower dispatch-on-enqueue path for the actual nudge | Partial | Screenshot of health strip | Low | Public, F4. Confirmed 2026-07-18: CCC's own code only flags stall from ground truth; the auto-nudge is Watchtower's. Updated 2026-08-21 (WT-92): no fallback caveat applies anymore — Watchtower is a hard dependency. |
...
3. "Source you can read in an afternoon" overstates: `app.js` is about 1 MB and
   ...
   model_advisor.py and others). Reframe honestly: no build
