...
2. **What did CCC do with it?** `~/.claude/command-center/logs/activity.log`,
   ...
   `RECOVER`, `RECOVER_GIVEUP`, `FORCE_RESTART`. No line at all = it vanished
   silently; say so, don't infer.
...
- `/api/inject-input` stamps `last-interactions.json` *before* delivery, so a
  recorded interaction does not mean the message was delivered.
