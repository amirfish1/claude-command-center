...
def _brief_filename(session_id):
    ...
    return f"kimi-session-{digest}.md"
...
def connect_total_recall(output_dir, executable="total-recall"):
    """Connect the exported folder through Total Recall's public CLI."""
    command = [str(executable), "ingest", str(Path(output_dir).expanduser())]
    try:
...
def connect_kimi_knowledge(output_dir, kimi_home=None, executable="total-recall"):
    """Sync first, then connect only a complete knowledge export."""
    result = sync_kimi_knowledge(output_dir, kimi_home=kimi_home)
    if result.errors:
        return result, None
    return result, connect_total_recall(output_dir, executable=executable)
