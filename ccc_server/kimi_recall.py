...
_AGGREGATE_FILE = "kimi-code-sessions.md"
...
def _aggregate_path(output_dir):
    return Path(output_dir) / _AGGREGATE_FILE


def _write_aggregate(output_dir, sessions):
    output_dir = Path(output_dir)
    documents = []
    for session_id in sorted(sessions):
        path = output_dir / _brief_filename(session_id)
        try:
            documents.append(path.read_text(encoding="utf-8"))
        except OSError:
            continue
    text = "# Kimi Code Knowledge Export\n\n"
    text += "This file is generated by CCC's Kimi Knowledge Bridge.\n"
    if documents:
        text += "\n\n---\n\n".join(documents)
    _atomic_write(_aggregate_path(output_dir), text)
...
def connect_total_recall(output_dir, executable="total-recall"):
    ...
    aggregate = _aggregate_path(Path(output_dir).expanduser())
    if not aggregate.is_file():
        return subprocess.CompletedProcess(
            [str(executable), "ingest", str(aggregate)], 1, "",
            f"Kimi knowledge export is missing: {aggregate}",
        )
    command = [str(executable), "ingest", str(aggregate)]
    try:
...
def sync_kimi_knowledge(output_dir, kimi_home=None):
    ...
    if exported or not _aggregate_path(output_dir).is_file():
        _write_aggregate(output_dir, state)
