...
- **Identity — read this carefully and follow exactly. Do not guess.**
  1. Find your full session id: (a) check `$ARGUMENTS` for a `sid="<uuid>"` parameter — the orchestrator always passes it explicitly; (b) if missing, run `echo $CLAUDE_SESSION_ID` via the Bash tool. **If both are empty, stop immediately**: post one `💬` saying "Cannot determine session id — `sid=` not in args and `$CLAUDE_SESSION_ID` is unset. Re-inject with `sid=<uuid>`." and exit. Do not attempt the old `.jsonl`-newest-file fallback — in multi-session environments that fallback picks whatever session was last active, which is almost never the correct one (this was OPS-50).
