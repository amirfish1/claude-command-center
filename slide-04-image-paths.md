# Slide #4 (UX-fix loop) — image paths on disk

From group chat check-in (`cbbec7d0`, 2026-06-07).

## Group chat post (paste into crafting-12-things-post chat)
...
No matched before/after BYM fix pair yet (nothing in `annotation-screenshots/` tied to `localhost:3001`). Fresh capture on BYM dev via `AnnotateButton` (`apps/bookyourmat/src/components/dev/AnnotateButton.tsx`) recommended.
