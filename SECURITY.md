...
- **`/api/open` sandbox** — the "open file in OS" endpoint resolves the requested path under the explicit repo/session context or command-center log directories and rejects anything outside. Default action is `open -R` (reveal in Finder), not launch.
