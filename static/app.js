(function() {
  ...
  function updateInputBar() {
    ...
    if (isConvTab && (hasSession || isNewSession || isBacklogIssue)) {
      ...
      if (isBacklogIssue) {
        ...
        $convInput.placeholder = 'Add edit instructions; submit to start session for issue #' + n + '…';
  ...
  async function sendToTerminal(paneId) {
    ...
    if (!text) return;
    // Backlog GH issue: typed text appended to the standard issue preamble,
    // then spawn a session — same outcome as clicking "Edit & start" on the
    // GH card and hitting Launch in the modal.
  ...
  async function loadConversationList() {
    try {
      ...
      if (!repoPath) {
        ...
        if (archiveMode) renderArchiveList($convSearch.value);
        return;
      }
  ...
  function renderSidebar(convs) {
    ...
    // Archive-mode guard: while "All repos" is on, the sidebar renders the
    // cross-folder archive instead of one concrete repo's session list. Skip
    // both kanban and list dispatch to keep the archive view from being
    // clobbered by periodic refreshes. Toggle off to return.
  ...
  function renderConversationList(convs) {
    ...
    const _renderRow = (c, opts = {}) => {
      ...
      const rowSizeHtml = isBacklogRow
        ? ''
      ...
      const historyBadgeHtml = c._historyMatch
        ? '<span class="conv-history-badge" title="Matched in conversation history (claude-index)">history</span>'
        : '';
      ...
      return '<div class="conv-item' + active + groupedRowClass + (isCodexRow ? ' is-codex' : '') + (isGeminiRow ? ' is-gemini' : '') + (c.pinned_repo ? ' is-repo-pinned' : '') + (c._historyMatch ? ' is-history-match' : '') + '" draggable="true" data-id="' + c.id + '" data-session-id="' + escapeHtml(c.session_id || c.id) + '" data-repo-path="' + rowRepoAttr + '">'
    ...
    if ($ghGroupingToggle) {
      $ghGroupingToggle.addEventListener('click', (ev) => {
        ...
        } else if (typeof loadConversationList === 'function') {
          loadConversationList();
        }
      });
    }
  ...
  function attachDragHandlers(el) {
    ...
    el.addEventListener('drop', async (ev) => {
      ...
      el.classList.remove('drop-above', 'drop-below');
      // Archive mode special-case: when src and dst belong to different
      // folder buckets, treat the drop as a repo pin instead of a reorder.
      // Lets the user grab a row from one repo group and drop it onto a
      // row in another group, not just onto the group header.
      if (typeof archiveMode !== 'undefined' && archiveMode) {
  ...
  function renderSessionUsageIntoStrip() {
    ...
    let modelPill = '';
    if (u.model) {
      const isOneM = (peak > 200000) || u.model.toLowerCase().includes('[1m]');
      const shortModel = u.model.replace(/^claude-/, '').replace(/\[1m\]/i, '').trim();
      const modelTip = u.model + (isOneM ? '\n(1M context window - anthropic-beta: context-1m)' : '');
      modelPill = ' <span class="wp-model-pill" title="' + escapeHtml(modelTip) + '">'
        + escapeHtml(shortModel) + (isOneM ? ' <span class="wp-model-1m">1M</span>' : '')
        + '</span>';
    }
    ...
    return fetch('/api/search-history?' + params.toString())
      ...
      .then(data => {
        ...
        for (const r of results) {
          ...
          if (!sid) continue;
          // First hit per session wins (FTS5 returns BM25-sorted, so the
          // best snippet is naturally first).
  ...
  function _appendAllReposPickerOption() {
    ...
    if (archiveMode) opt.selected = true;
  ...
  // ── All-repos archive mode ──────────────────────────────────────────────
  // Read-only browse of every conversation across every folder under
  // ~/.claude/projects/. This is now the normal sidebar data source; the
  // folder dropdown narrows this archive locally instead of switching repos.
  ...
  async function refreshGhIssuesSection() {
    ...
    try {
      if (archiveMode) renderArchiveList(query);
      if (archiveMode) {
      ...
      if (archiveMode) renderArchiveList(document.getElementById('convSearch')?.value || query);
    }
  }
  ...
  function renderArchiveList(filter) {
    ...
    const byFolder = archiveFolderFilter === ARCHIVE_FOLDER_ALL
      ? archiveRows
    ...
    // c._historyMatch / c._historySnippet. Skipped when state is stale
    // (debounced fetch hasn't completed for this keystroke yet).
    if (q && _historyState.query === q && _historyState.map.size) {
  ...
  async function setArchiveMode(on, opts = {}) {
    archiveMode = true;
    try { localStorage.setItem(_ARCHIVE_MODE_KEY, '1'); } catch (_) {}
    updateRepoPickerVisibility();
    const $list = document.getElementById('convList');
    ...
    if (archiveMode) {
      ...
      // #convList slot. We don't toggle the existing convKanbanToggle's
      // state — that's user-owned. Just visually hide the board so the
      // archive list takes the full sidebar.
      if ($kanban) $kanban.style.display = 'none';
      if ($list) {
        $list.style.display = '';
      ...
      await refreshArchiveData();
      // The user may have toggled off while loading — guard before render.
      if (archiveMode) renderArchiveList(document.getElementById('convSearch')?.value || '');
    } else {
      // Restore: re-trigger the normal session-list load.
      if (opts.reloadCurrent !== false) {
  ...
  (function wireArchiveMode() {
    ...
    const $search = document.getElementById('convSearch');
    if ($search) {
      // Piggyback on existing search input — when in archive mode we
      // re-render with the filter; when not, leave the existing list-
      // filter logic alone (which is wired elsewhere via its own listener).
      $search.addEventListener('input', () => {
        if (archiveMode) renderArchiveList($search.value);
      });
    }
    // Boot kick: if archive is the saved/default mode, wait for the
    // first /api/sessions response before kicking the archive walk.
    // Both endpoints share CPU/subprocess slots in the same Python
    // process; running the slow one (cross-folder JSONL + git ops,
    // ~12s on a cold cache) in parallel with /api/sessions made the
    // overlay linger because /api/sessions itself was getting
    // starved. Waiting until sessions returns means the selected repo
    // is interactive in <1s, then the archive populates the sidebar.
    if (archiveMode) {
      ...
      _firstSessionsLoaded.then(() => {
        // The user may have toggled archive off during the wait.
        if (archiveMode) setArchiveMode(true);
      });
    }
  })();
